...
def spell_names(raw_spells: Dict[str, Any]) -> List[str]:
    ...
    for spell_key in ("summonerSpellOne", "summonerSpellTwo"):
        ...
        display_name = str(spell_data.get("displayName") or spell_data.get("rawDisplayName") or "").strip()
        if display_name:
...
def has_smite(raw_spells: Dict[str, Any]) -> bool:
    ...
    return any("smite" in spell_name.lower() for spell_name in spell_names(raw_spells))
